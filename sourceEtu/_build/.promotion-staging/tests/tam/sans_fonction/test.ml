...
let%expect_test "testif1" =
  ...
  [%expect{| Semantic error: asm.SemanticError: L'étiquette 'label4' n'est pas définie. |}]

let%expect_test "testif2" =
  runtam (pathFichiersRat^"testif2.rat");
  [%expect{| Semantic error: asm.SemanticError: L'étiquette 'label6' n'est pas définie. |}]
